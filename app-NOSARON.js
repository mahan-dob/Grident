...
const CONFIG = {
  ...
  colors: {
    palette: ['#ff0066', '#00ff88', '#7c3aed', '#ff6600', '#00d4ff', '#ffdd00', '#ff3366', '#00ffcc']
  }
};
...
// Resize states
let resizingW = false, resizingH = false;
let startX = 0, startY = 0, startW = 0, startH = 0;
let lastW = 0, lastH = 0;

// Picker drag states
let sbDrag = false, hueDrag = false, alphaDrag = false;
...
function isMobile() {
  return window.innerWidth <= CONFIG.mobile.breakpoint || 'ontouchstart' in window;
}
...
function randColor() {
  return CONFIG.colors.palette[Math.floor(Math.random() * CONFIG.colors.palette.length)];
}
...
function applyAspectRatio(adjustHeight = true) {
  ...
  if (adjustHeight) {
    state.canvasHeight = Math.round(state.canvasWidth / dimensionState.aspectRatio);
    state.canvasHeight = clamp(state.canvasHeight, CONFIG.canvas.minHeight, CONFIG.canvas.maxHeight);
    
    if (state.canvasHeight === CONFIG.canvas.minHeight || state.canvasHeight === CONFIG.canvas.maxHeight) {
      state.canvasWidth = Math.round(state.canvasHeight * dimensionState.aspectRatio);
      state.canvasWidth = clamp(state.canvasWidth, CONFIG.canvas.minWidth, CONFIG.canvas.maxWidth);
    }
  } else {
    state.canvasWidth = Math.round(state.canvasHeight * dimensionState.aspectRatio);
    state.canvasWidth = clamp(state.canvasWidth, CONFIG.canvas.minWidth, CONFIG.canvas.maxWidth);
    
    if (state.canvasWidth === CONFIG.canvas.minWidth || state.canvasWidth === CONFIG.canvas.maxWidth) {
      state.canvasHeight = Math.round(state.canvasWidth / dimensionState.aspectRatio);
      state.canvasHeight = clamp(state.canvasHeight, CONFIG.canvas.minHeight, CONFIG.canvas.maxHeight);
    }
  }
...
function swapDimensions() {
  ...
  if (dimensionState.aspectW !== null && dimensionState.aspectH !== null) {
    ...
    dimensionState.aspectRatio = dimensionState.aspectW / dimensionState.aspectH;
  }
...
function handleWidthChange(newWidth, fromInput = false) {
  newWidth = clamp(parseInt(newWidth) || CONFIG.canvas.minWidth, CONFIG.canvas.minWidth, CONFIG.canvas.maxWidth);
...
function handleHeightChange(newHeight, fromInput = false) {
  newHeight = clamp(parseInt(newHeight) || CONFIG.canvas.minHeight, CONFIG.canvas.minHeight, CONFIG.canvas.maxHeight);
...
function checkAndSetMatchingPresets() {
  ...
  if (dimensionState.aspectW && dimensionState.aspectH) {
    dimensionState.activeAspectPreset = null;
    for (const [name, preset] of Object.entries(aspectPresets)) {
      if (preset.w === dimensionState.aspectW && preset.h === dimensionState.aspectH) {
        dimensionState.activeAspectPreset = name;
        break;
      }
    }
...
function updateAspectButtonsUI() {
  document.querySelectorAll('.aspect-btn').forEach(btn => {
    ...
    if (ratio === 'free' && !dimensionState.aspectLocked) {
      isActive = true;
    } else if (ratio === dimensionState.activeAspectPreset && dimensionState.aspectLocked) {
      isActive = true;
    }
...
function updateLockButtonUI() {
  ...
  if (lockIcon) {
    lockIcon.src = dimensionState.aspectLocked ? './icon/lock.svg' : './icon/unlock.svg';
  }
...
function updateSizeDisplay() {
  ...
  if (el) {
    el.textContent = `${Math.round(state.canvasWidth)} × ${Math.round(state.canvasHeight)}`;
  }
}
...
function checkAndFixZoom() {
  ...
  const tooLarge = displayWidth > rect.width - padding || 
                   displayHeight > rect.height - padding;
  
  const tooSmall = displayWidth < (rect.width - padding) * 0.93 && 
...
function setCanvasSize(w, h) {
  state.canvasWidth = Math.floor(clamp(w, CONFIG.canvas.minWidth, CONFIG.canvas.maxWidth));
  state.canvasHeight = Math.floor(clamp(h, CONFIG.canvas.minHeight, CONFIG.canvas.maxHeight));
  refresh();
}
...
function toggleVis(id) {
  const s = getStop(id);
  if (s) { s.visible = !s.visible; refresh(); }
}

function addColorStop(s) {
  const pos = s.stops.length < 5 ? Math.round(100 / (s.stops.length + 1)) * s.stops.length : 50;
...
function delColorStop(s, i) {
  if (s.stops.length > 2) { s.stops.splice(i, 1); refresh(); }
}
// ========== TOGGLE HANDLES ==========
...
function drawLinearHandle(s, cx, cy, sel) {
  ...
  if (s.stops.length >= 2) {
    ...
    lineGrad.addColorStop(0, sel ? 'rgba(255,255,255,0.6)' : 'rgba(255,255,255,0.3)');
    lineGrad.addColorStop(1, sel ? 'rgba(255,255,255,0.6)' : 'rgba(255,255,255,0.3)');
  }
  ...
  s.stops.forEach((cs, i) => {
    ...
    if (sel) {
      drawStopLabel(px, py + handleSize + (12 * scale), cs.pos + '%', fontSize, scale);
    }
  });
...
function drawColorStop(px, py, cs, sel, handleSize, lineWidth, scale) {
  ...
  const innerGrad = ctx.createRadialGradient(
    px - handleSize * 0.2, py - handleSize * 0.2, 0,
    px, py, handleSize * 0.6
  );
...
function drawStopLabel(x, y, text, fontSize, scale) {
  ...
  ctx.beginPath();
  ctx.roundRect(x - bgWidth / 2, y - bgHeight / 2, bgWidth, bgHeight, 4 * scale);
  ctx.fill();
...
function renderRecentColors() {
  ...
  if (recentColors.length === 0) {
    container.innerHTML = '<span class="recent-colors-empty">No recent colors</span>';
    return;
  }
  
  container.innerHTML = recentColors.map((c, i) => `
    <div class="recent-color-item" 
         onclick="selectRecentColor(${i})" 
         title="${c.hex} (${c.alpha}%)">
      <div class="recent-color-inner" style="background: ${rgba(c.hex, c.alpha / 100)}"></div>
    </div>
  `).join('');
}
...
function updatePicker() {
  ...
  if (sbCursor) {
    sbCursor.style.cssText = `left: ${s}%; top: ${100 - v}%; background: ${hex}`;
  }
  if (hueThumb) {
  ...
  document.querySelectorAll('.format-btn').forEach(b => {
    b.classList.toggle('active', b.dataset.f === fmt);
  });
    ...
    if (fmt === 'hex') {
      f.innerHTML = `
        ...
        <div class="picker-field">
          <label>A</label>
          <input type="number" id="fA" min="0" max="100" value="${Math.round(a)}">
        </div>`;
    } else if (fmt === 'rgb') {
      f.innerHTML = `
        <div class="picker-field"><label>R</label><input type="number" id="fR" min="0" max="255" value="${rgb.r}"></div>
        <div class="picker-field"><label>G</label><input type="number" id="fG" min="0" max="255" value="${rgb.g}"></div>
        <div class="picker-field"><label>B</label><input type="number" id="fB" min="0" max="255" value="${rgb.b}"></div>
        <div class="picker-field"><label>A</label><input type="number" id="fA" min="0" max="100" value="${Math.round(a)}"></div>`;
    } else {
      f.innerHTML = `
        <div class="picker-field"><label>H</label><input type="number" id="fH" min="0" max="360" value="${hsl.h}"></div>
        <div class="picker-field"><label>S</label><input type="number" id="fS" min="0" max="100" value="${hsl.s}"></div>
        <div class="picker-field"><label>L</label><input type="number" id="fL" min="0" max="100" value="${hsl.l}"></div>
        <div class="picker-field"><label>A</label><input type="number" id="fA" min="0" max="100" value="${Math.round(a)}"></div>`;
    }
...
function initPickerEvents() {
  ...
  document.addEventListener('touchmove', e => {
    if (sbDrag) { e.preventDefault(); updSB(e); }
    if (hueDrag) { e.preventDefault(); updHue(e); }
    if (alphaDrag) { e.preventDefault(); updAlpha(e); }
  }, { passive: false });
  
  document.addEventListener('mouseup', () => {
    sbDrag = hueDrag = alphaDrag = false;
    setTimeout(() => { pickerDragging = false; }, 50);
  });
  ...
  if (pickerModal) {
    ...
    pickerModal.addEventListener('touchstart', e => e.stopPropagation(), { passive: true });
  }
}
...
function getGradPreview(s) {
  if (s.type === 'radial') return `radial-gradient(circle, ${rgba(s.color, s.opacity/100)}, transparent)`;
  if (s.type === 'conic') return `conic-gradient(from ${s.startAngle}deg, ${s.stops.map(c => rgba(c.color, c.opacity/100) + ' ' + c.pos + '%').join(', ')})`;
  return `linear-gradient(${s.angle}deg, ${s.stops.map(c => rgba(c.color, c.opacity/100) + ' ' + c.pos + '%').join(', ')})`;
}
...
function renderList() {
  ...
  el.innerHTML = state.stops.map(s => `
    <div class="stop-item ${state.selected === s.id ? 'selected' : ''} ${!s.visible ? 'hidden' : ''}" onclick="state.selected='${s.id}';refresh()">
      <div class="stop-header">
        <div class="stop-preview"><div class="stop-preview-inner" style="background:${getGradPreview(s)}"></div></div>
        <div class="stop-info">
          <div class="stop-name">${s.name}</div>
          <div class="stop-meta">${s.type} · ${s.type === 'radial' ? Math.round(s.size) + 'px' : s.type === 'conic' ? s.startAngle + '°' : s.angle + '°'}</div>
        </div>
        <div class="stop-actions">
          <button class="icon-btn" onclick="event.stopPropagation();toggleVis('${s.id}')">${s.visible ? '<img src="./icon/eye.svg" alt="eye">' : '<img src="./icon/eye-close.svg" alt="eye-close">'}</button>
          <button class="icon-btn" onclick="event.stopPropagation();dupStop('${s.id}')"><img src="./icon/copy.svg" alt="copy"></button>
          <button class="icon-btn" onclick="event.stopPropagation();delStop('${s.id}')"><img src="./icon/close.svg" alt="close"></button>
        </div>
      </div>
    </div>
  `).join('');
...
function renderInspector() {
  ...
  let h = `
    <div class="form-group">
      ...
      <div class="form-row">
        <label>Name</label>
        <input class="num-input" style="width:100%;text-align:left" value="${s.name}" onchange="getStop('${s.id}').name=this.value;refresh()">
      </div>
      <div class="form-row">
        <label>X</label>
        <input type="number" class="num-input" min="0" max="100" value="${Math.round(s.x * 100)}" 
          oninput="getStop('${s.id}').x = +this.value / 100; draw(); updateCSS()">
      </div>
      <div class="form-row">
        <label>Y</label>
        <input type="number" class="num-input" min="0" max="100" value="${Math.round(s.y * 100)}" 
          oninput="getStop('${s.id}').y = +this.value / 100; draw(); updateCSS()"
  ...
  if (s.type === 'radial') {
    h += `
      <div class="form-group">
        ...
        <div class="form-row">
          <div class="color-swatch" onclick="openPicker('${s.color}',${s.opacity},(c,a)=>{getStop('${s.id}').color=c;getStop('${s.id}').opacity=a;refresh()})">
        ...
        <div class="form-row">
          <label>Opacity</label>
          <input type="number" class="num-input" min="0" max="100" value="${Math.round(s.opacity)}" 
  ...
  if (s.type === 'conic') {
    h += `
      <div class="form-group">
        <div class="form-group-title">Start Angle</div>
        <div class="form-row" style="align-items: center; gap: 15px;">
        ...
        ${s.stops.map((cs, i) => `
          <div class="color-stop-row">
            <div class="color-stop-header">
              <div class="color-swatch" onclick="openPicker('${cs.color}',${cs.opacity},(c,a)=>{getStop('${s.id}').stops[${i}].color=c;getStop('${s.id}').stops[${i}].opacity=a;refresh()})">
            ...
            <div class="color-stop-fields">
              <div class="field-group">
                ...
                <input type="number" class="num-input" min="0" max="100" value="${cs.pos}" 
                  oninput="getStop('${s.id}').stops[${i}].pos=+this.value;draw();updateCSS()">
              </div>
              <div class="field-group">
                <label>Opacity</label>
                <input type="number" class="num-input" min="0" max="100" value="${cs.opacity}" 
                  oninput="getStop('${s.id}').stops[${i}].opacity=+this.value;draw();updateCSS()">
              </div>
            </div>
            ${s.stops.length > 2 ? `<button class="icon-btn" onclick="delColorStop(getStop('${s.id}'),${i})"><img src="./icon/close.svg" alt="delete color"></button>` : ''}
          </div>
        `).join('')}
      </div>
    `;
  ...
  if (!vis.length) {
    ...
    const grads = vis.map(s => {
      if (s.type === 'radial') {
        ...
        const cs = sorted.map(c => `${formatColor(c.color, c.opacity, fmt)} ${c.pos}%`).join(', ');
...
function highlight(css) {
  return css
    ...
    .replace(/(background(?:-color|-image|-blend-mode)?)\s*:/g,'<span class="p">$1</span>:')
    .replace(/(radial-gradient|linear-gradient|conic-gradient)/g,'<span class="f">$1</span>')
...
function exportAsImage(format = 'png', quality = 0.92) {
  ...
  state.stops.filter(s => s.visible).forEach(s => {
    ...
    if (s.type === 'radial') {
      const outer = s.size;
      const solidEnd = 1 - s.feather / 100;
      const grad = exportCtx.createRadialGradient(cx, cy, 0, cx, cy, outer);
      const color = rgba(s.color, s.opacity / 100);
      
      grad.addColorStop(0, color);
      if (solidEnd > 0 && solidEnd < 1) grad.addColorStop(solidEnd, color);
      grad.addColorStop(1, rgba(s.color, 0));
      
      exportCtx.fillStyle = grad;
      exportCtx.beginPath();
      exportCtx.arc(cx, cy, outer, 0, Math.PI * 2);
      exportCtx.fill();
      ...
      const grad = exportCtx.createLinearGradient(mcx - dx, mcy - dy, mcx + dx, mcy + dy);
      ...
      const startAngle = ((s.startAngle - 90) * Math.PI) / 180;
      const grad = exportCtx.createConicGradient(startAngle, cx, cy);
      [...s.stops].sort((a, b) => a.pos - b.pos).forEach(cs => {
        grad.addColorStop(clamp(cs.pos / 100, 0, 1), rgba(cs.color, cs.opacity / 100));
      });
...
function exportAsSVG() {
  ...
  elements += `  <rect width="100%" height="100%" fill="rgba(${bgRgb.r},${bgRgb.g},${bgRgb.b},${state.bgAlpha/100})"/>\n`;
  ...
  vis.forEach((s, idx) => {
    ...
    if (s.type === 'radial') {
      ...
      defs += `    <radialGradient id="${gradId}" gradientUnits="userSpaceOnUse" cx="${s.x * width}" cy="${s.y * height}" r="${s.size}">\n`;
      defs += `      <stop offset="0%" stop-color="rgb(${rgb.r},${rgb.g},${rgb.b})" stop-opacity="${s.opacity/100}"/>\n`;
      if (solidEnd > 0 && solidEnd < 1) {
        defs += `      <stop offset="${(solidEnd * 100).toFixed(0)}%" stop-color="rgb(${rgb.r},${rgb.g},${rgb.b})" stop-opacity="${s.opacity/100}"/>\n`;
      }
      defs += `      <stop offset="100%" stop-color="rgb(${rgb.r},${rgb.g},${rgb.b})" stop-opacity="0"/>\n`;
      defs += `    </radialGradient>\n`;
      
      elements += `  <circle cx="${s.x * width}" cy="${s.y * height}" r="${s.size}" fill="url(#${gradId})" style="mix-blend-mode:screen"/>\n`;
      ...
      [...s.stops].sort((a, b) => a.pos - b.pos).forEach(cs => {
        ...
        defs += `      <stop offset="${cs.pos}%" stop-color="rgb(${rgb.r},${rgb.g},${rgb.b})" stop-opacity="${cs.opacity/100}"/>\n`;
      });
      defs += `    </linearGradient>\n`;
      
      elements += `  <rect width="100%" height="100%" fill="url(#${gradId})" style="mix-blend-mode:screen"/>\n`;
      
    } else if (s.type === 'conic') {
      const x = (s.x * 100).toFixed(1);
      const y = (s.y * 100).toFixed(1);
      const stops = [...s.stops].sort((a, b) => a.pos - b.pos)
        .map(cs => {
          const rgb = hexToRgb(cs.color);
          return `rgba(${rgb.r},${rgb.g},${rgb.b},${cs.opacity/100}) ${cs.pos}%`;
        }).join(', ');
      
      elements += `  <foreignObject x="0" y="0" width="100%" height="100%" style="mix-blend-mode:screen">
...
${elements}</svg>`;
  ...
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.download = `gradient-${width}x${height}.svg`;
  link.href = url;
  link.click();
  URL.revokeObjectURL(url);
}
...
function zoomIn() { setZoom(zoomState.current + zoomState.step); }
function zoomOut() { setZoom(zoomState.current - zoomState.step); }
...
function checkAndFixZoom() {
  ...
  const tooLarge = displayWidth > rect.width - padding || displayHeight > rect.height - padding;
  const tooSmall = displayWidth < (rect.width - padding) * 0.3 && displayHeight < (rect.height - padding) * 0.3;
...
function setupKeyboardZoom() {
  document.addEventListener('keydown', e => {
    ...
    switch(e.key.toLowerCase()) {
      case 'f':
        if (!isMod) { e.preventDefault(); fitToScreen(); }
        break;
      case 'h':
        if (!isMod) { handleToggleClick(e); }
        break;
      case 'x':
        if (!isMod) { e.preventDefault(); swapDimensions(); }
        break;
      case 'l':
        if (!isMod) { e.preventDefault(); toggleAspectLock(); }
        break;
      case 'delete':
      case 'backspace':
        if (state.selected) { e.preventDefault(); delStop(state.selected); }
        break;
      case 'escape':
        state.selected = null;
        closePicker();
...
document.addEventListener('mousemove', e => {
  ...
  if (resizingW) {
    let newH = clamp(startH + (e.clientY - startY), CONFIG.canvas.minHeight, CONFIG.canvas.maxHeight);
    if (newH !== lastH) {
      lastH = newH;
  ...
  if (resizingH) {
    let newW = clamp(startW + (e.clientX - startX), CONFIG.canvas.minWidth, CONFIG.canvas.maxWidth);
    if (newW !== lastW) {
      lastW = newW;
      state.canvasWidth = newW;
...
document.addEventListener('touchmove', e => {
  ...
  if (resizingW) {
    let newH = clamp(startH + (clientY - startY), CONFIG.canvas.minHeight, CONFIG.canvas.maxHeight);
    if (newH !== lastH) {
      lastH = newH;
      state.canvasHeight = newH;
  ...
  if (resizingH) {
    let newW = clamp(startW + (clientX - startX), CONFIG.canvas.minWidth, CONFIG.canvas.maxWidth);
    if (newW !== lastW) {
      lastW = newW;
      state.canvasWidth = newW;
...
function initDimensionEvents() {
  ...
  if (aspectW && aspectH) {
    ...
    aspectW.addEventListener('keydown', e => {
      if (e.key === 'Enter') { e.preventDefault(); aspectH.focus(); }
      if (e.key === 'Escape') aspectW.blur();
    });
    
    aspectH.addEventListener('keydown', e => {
      if (e.key === 'Enter') { e.preventDefault(); handleAspectInput(); aspectH.blur(); }
  ...
  // Lock button
  document.getElementById('aspectLockBtn')?.addEventListener('click', toggleAspectLock);
  document.getElementById('sizeLinkBtn')?.addEventListener('click', toggleAspectLock);
  
  // Swap button
  document.getElementById('swapSizeBtn')?.addEventListener('click', swapDimensions);
  ...
  if (canvasWidth) {
    canvasWidth.addEventListener('input', () => {
      handleWidthChange(canvasWidth.value, true);
    });
  ...
  if (canvasHeight) {
    canvasHeight.addEventListener('input', () => {
      handleHeightChange(canvasHeight.value, true);
    });
...
document.getElementById('btnRadial')?.addEventListener('click', () => addStop('radial'));
document.getElementById('btnLinear')?.addEventListener('click', () => addStop('linear'));
document.getElementById('btnConic')?.addEventListener('click', () => addStop('conic'));
