  ...
  visibleStops.forEach((s, i) => {
    ...
    if (s.type === 'radial') {
      ...
      defs += `<radialGradient id="${id}" gradientUnits="userSpaceOnUse"
        ...
        ${solidEnd > 0 && solidEnd < 1
          ? `<stop offset="${solidEnd * 100}%" stop-color="rgb(${rgb.r},${rgb.g},${rgb.b})" stop-opacity="${s.opacity / 100}"/>`
          : ''
        }
        <stop offset="100%" stop-color="rgb(${rgb.r},${rgb.g},${rgb.b})" stop-opacity="0"/>
      </radialGradient>\n`;

      content += `<circle cx="${s.x * width}" cy="${s.y * height}" r="${s.size}" fill="url(#${id})" style="mix-blend-mode:screen"/>\n`;
    }
    ...
    // ===== LINEAR =====
      ...
      [...s.stops].sort((a, b) => a.pos - b.pos).forEach(cs => {
        ...
        defs += `<stop offset="${cs.pos}%" stop-color="rgb(${c.r},${c.g},${c.b})" stop-opacity="${cs.opacity / 100}"/>\n`;
      });
      defs += `</linearGradient>\n`;
